...
export default function EditReport() {
  ...
  useEffect(() => {
    const loadPost = async () => {
      ...
      // Fall back to "moderate" if existing post pre-dates the severity column
      setSeverity((post.severity as Severity) ?? "moderate");
      // Fall back to "active" if existing post pre-dates the status column
    ...
    return (
      <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold text-slate-500">Loading flood report...</p>
        </div>
      </main>
    );
  }
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-6">
              ...
              {imageUrl && (
                <div>
                  ...
                  <div className="rounded-2xl overflow-hidden border border-slate-200 bg-slate-900 max-h-56">
                    <img
                      src={imageUrl}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {([
                    ...
                    return (
                      <label
                        key={value}
                        className={`flex items-start gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all duration-100 select-none ${
                          isSelected
                            ? `${config.bg} ${config.border} ${config.text}`
                            : "bg-white border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50"
                        }`}
              ...
              <div className="pt-2 flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => router.push("/")}
                  className="flex-1 py-3 text-center text-xs font-bold uppercase tracking-wider text-slate-600 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
